...
## Phase 3: controlled mathematical studies

`engine/studies.ts` constructs independent settings snapshots for seven transforms, the Sigmoid/Softmax pair, or budgets .9/.7/.5/.3. The `study` worker action invokes the same `runExperiment` with `hybrid` for every member, honoring draft weights and lexical/embedding relevance. Only the named variable changes. Before returning, the study verifies the same original/hash, engine version, full resolved focus, all fixed settings, raw feature breakdowns/scores, chunk identities/protection and isolated costs. No strategy or scoring formula was changed for the teaching example.

React receives one complete group and appends its runs atomically. Existing generation/worker-identity cancellation applies; no partial or late group is published. The comparison derives from recorded runs, with study identity/index/total as optional schema-2 metadata. No second original snapshot or separately cached comparison table is added. JSON redaction still applies to every text-bearing run field; CSV adds study ID/kind/variable/index/total. The existing 100-run cap can evict part of an old group; the comparison then reports incomplete history instead of presenting a partial study as complete.

`StudyResults` shows actual full-string counts, budget status, protection, measured cosine or missing values, and same-chunk Sigmoid/Softmax decisions. `studyObservations` generates deterministic descriptions from those records. Selecting a level/row opens that recorded result's diff. The swept variable is excluded from the stale-study-controls notice; fixed-control edits require a new study. `TransformChart` uses the actual transform function for independent curves and a separate four-score Softmax distribution illustration. No fake similarity, task-performance score, neural perplexity or composite was added. The engine evidence version is `tokenlab-0.3.0`.

Formulas, actual teaching results, cutoff semantics and limits: [MATH-EXPERIMENTS](MATH-EXPERIMENTS.md).
