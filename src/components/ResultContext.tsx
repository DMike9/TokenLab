...
export function ResultContext({ run, changed }: { run: Run; changed: boolean }) {
    ...
    return <>
        ...
        {run.budgetMet === false && <p className="notice result-explanation">{run.stages.at(-1)?.notes.some(note => note.startsWith('Candidate rejected')) ? 'Target budget not met: a rejected candidate was restored to the previous stage text, which exceeds this target.' : run.stages.at(-1)?.method === 'similarity' ? 'Target budget not met: protected chunks and/or the semantic floor prevented further deletion.' : 'Target budget not met: detected protected chunks exceed the target. Protection takes priority; increase the retained budget to allow more text.'} This is a constraint outcome, not an application failure.</p>}
    </>;
}
